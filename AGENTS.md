...
- New dependencies, fonts, images, models, SDKs, payment components, and other bundled assets MUST have licenses and terms compatible with the intended commercial use and AGPLv3 distribution. Update `THIRD-PARTY-LICENSES.md` when the distributed dependency set changes. Do not commercially release while a required dependency has proprietary, missing, or otherwise unverified redistribution rights; `github.com/Calcium-Ion/go-epay v0.0.4` is currently such a dependency and must be covered by written permission or replaced with a clearly licensed alternative before release.
