...
**Commercial derivative and AGPLv3 compliance:** This repository is a commercial derivative of New API operated under the GNU Affero General Public License v3.0. Commercial use, paid hosting, subscriptions, support, and distribution are allowed only while the following rules remain satisfied:

- The covered work and all modifications to it MUST remain licensed under AGPLv3 unless a separate written commercial license from the relevant copyright holders explicitly permits otherwise. Do not relicense the covered work as MIT, Apache-2.0, a proprietary license, or any other incompatible license.
- Keep `LICENSE`, `THIRD-PARTY-LICENSES.md`, copyright notices, license headers, warranty notices, legal notices, and upstream attribution intact. Do not delete, hide, weaken, or replace them.
- Modified versions MUST prominently state that the software was modified and record the relevant modification date. Release notes or another durable change record must identify material derivative changes.
- The project's chosen AGPL network-use compliance mechanism is a public source repository. Users interacting with a deployed service MUST have a clear, prominent, and free "Source Code" link to the complete corresponding source for the exact version running in production. The derivative repository is `https://github.com/xyzxiaoma/ak47-api`; link to an exact deployed tag or commit whenever possible.
- Complete corresponding source includes the modified backend and frontend, database migrations, dependency manifests, and the scripts/configuration needed to build, install, and run that version. Secrets, API keys, passwords, private certificates, production data, and other credentials MUST NOT be committed or published.
- Docker images, binaries, installers, or other distributed artifacts MUST include or accompany the AGPLv3 license and applicable third-party notices, and recipients must be able to obtain the complete corresponding source for that exact artifact version.
- Every user interface MUST preserve the exact attribution notice `Frontend design and development by New API contributors.` in the appropriate legal notices and in a prominent about, legal, footer, or attribution location. It MUST also preserve a visible link to the original project at `https://github.com/QuantumNous/new-api`.
- Derivative branding may be added only when it does not remove, replace, obscure, or imply official endorsement by New API or QuantumNous. The product must not be presented as an official upstream release.
- A public repository by itself is not sufficient. Before each commercial deployment or release, verify that the published source actually matches the deployed artifact, that the source link is reachable by service users, and that the deployment is tagged or otherwise pinned to a reproducible commit.
- New dependencies, fonts, images, models, SDKs, payment components, and other bundled assets MUST have licenses and terms compatible with the intended commercial use and AGPLv3 distribution. Update `THIRD-PARTY-LICENSES.md` when the distributed dependency set changes. For unchanged dependencies inherited from New API, preserve and follow the upstream license and dependency notices. On 2026-09-06 the operator explicitly removed the additional local written-permission release gate for the inherited `github.com/Calcium-Ion/go-epay v0.0.4` dependency and directed release under New API's upstream requirements. This operator policy decision is not evidence of a new license grant; do not relabel the upstream `Proprietary/Internal` entry as an open-source license or claim separate written permission was obtained.
- Upstream AI model and API terms are separate from this repository's license. Commercial operation MUST independently verify resale/service rights and applicable requirements for privacy, content safety, user identity, logging, payments, tax, filings, and other jurisdiction-specific obligations.
- If a requested change would make the derivative closed-source, remove required attribution, hide the original-project link, publish secrets, or otherwise conflict with these rules, refuse the change and explain that a separate commercial license and legal review are required.
